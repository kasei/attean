...
my $t	= Attean::ValueExpression->new( value => Attean::Literal->true );
my $f	= Attean::ValueExpression->new( value => Attean::Literal->false );

{
	my $e	= Attean::BinaryExpression->new( children => [$t, $f], operator => '&&' );
	ok($e->does('Attean::API::Expression'));
	is($e->as_string, '("true"^^<http://www.w3.org/2001/XMLSchema#boolean> && "false"^^<http://www.w3.org/2001/XMLSchema#boolean>)', 'binary &&');
}

{
	my $e	= Attean::UnaryExpression->new( children => [$f], operator => 'not' );
	ok($e->does('Attean::API::Expression'));
	is($e->as_string, 'not("false"^^<http://www.w3.org/2001/XMLSchema#boolean>)', 'unary not');
}
...

{
	...
	ok($e->does('Attean::API::Expression'));
	is($e->as_string, 'coalesce("false"^^<http://www.w3.org/2001/XMLSchema#boolean>, "true"^^<http://www.w3.org/2001/XMLSchema#boolean>)', 'function coalesce');
}
