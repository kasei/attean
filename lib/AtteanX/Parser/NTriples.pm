...
package AtteanX::Parser::NTriples 0.001 {
	...
	sub canonical_media_type { return "text/ntriples" }
	sub media_types {
		return [qw(text/ntriples)];
	}
	has 'handled_type' => (
		is => 'ro',
	...
	sub parse_iter_from_io {
		...
		my $gen		= sub {
			while (defined($line = <$fh>)) {
				...
				my $triple	= $self->_statement( \@nodes, $lineno );
		...
		return Attean::CodeIterator->new(
			generator => $gen,
			item_type => Moose::Meta::TypeConstraint::Role->new(role => 'Attean::API::Triple'),
		);
	}
