...
my $path	= File::Spec->catfile( $Bin, 'data', 'rdf-tests', 'rdf', 'rdf12', 'rdf-turtle', 'syntax' );
my @good;
my @bad;
find( sub {
	return if ($File::Find::name =~ /^manifest[.]ttl/);
	if ($File::Find::name =~ /^(.*)[.]ttl/) {
		my $prefix	= $1;
		my ($file)	= $File::Find::name =~ m{rdf-turtle/syntax(.*)$};
		if ($file =~ /\bbad\b/) {
	...
	TODO: {
		...
		if ($file =~ $ok_regex) {
			...
			my (undef, undef, $test)	= File::Spec->splitpath( $file );
			{
				...
				lives_ok {
					my ($filename)	= $file;
					my $url	= 'https://github.com/w3c/rdf-star/blob/main/tests/turtle/syntax/' . $filename;

					my $parser = Attean->get_parser('Turtle')->new(base => $url);
